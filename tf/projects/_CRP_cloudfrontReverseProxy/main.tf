...
module "distribution" {
  ...
  distributionDefaultCacheBehavior = merge({
    cache_policy_id          = module.cachePolicy.cachePolicyId[0]
    origin_request_policy_id = module.orpDataSource.orpId[0]
  }, var.CRP_DistributionDefaultCacheBehavior)
  ...
  distributionOrderedCacheBehavior = concat([
    merge({
      cache_policy_id          = module.cachePolicy.cachePolicyId[0]
      origin_request_policy_id = module.orp.orpId[0]
    }, var.CRP_DistributionInitialOrderedCacheBehavior)
  ], var.CRP_DistributionOrderedCacheBehavior)
